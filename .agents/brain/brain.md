...
- Tree config requires n_steps >= 1 for prices and >= 2 for Greeks (gamma and theta read step-2 nodes), and scheme == "crr"; the same applies to the American tree engine, whose Greeks use the same estimators (`lattice_delta_gamma_theta`); the CRR lattice raises `InvalidInputError` when no-arbitrage fails. Deterministic for fixed inputs. (source: src/qpl/engines/tree/pricers.py; src/qpl/engines/tree/lattice.py; tests/test_tree_pricing.py)
...
- Next slice: Leisen-Reimer smoothing checked by measured order 2 — now better motivated, since Slice 2 measured that Richardson extrapolation is not a substitute on American payoffs. (source: docs/CURRICULUM.md)
